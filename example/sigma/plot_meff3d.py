...
# for (mdat, merr, label) in zip(m_QMC, m_QMC_errs, labels):
    ...
    # idx = idx + 1
handle1 = errorbar_mvsrs(rs_DMC, m_DMC, m_DMC_err, 0, "DMC [12]", zorder=10)
handle2 = errorbar_mvsrs(rs_VMC, m_SJVMC, m_SJVMC_err, 1, "VMC [13]", zorder=20)
...
for (rsdat, mdat, label) in zip(rsdats, mdats, labels):
    print("\nPlotting ", label)
    handle = plot_mvsrs(rsdat, mdat, idx, label, '--', rs_HDL=rs_HDL, meff_HDL=meff_HDL)
...
labels = [r"$G_0 W_0$", r"$G_0 W_+$", r"$G_0 W_\\pm$"]
# labels = [r"RPA"]
# labels = [r"$G_0 W_0$"]
# labels = [r"$G_0 W_0$", r"$G_+$ [15]", r"$G_+$\,\&\,$G_-$ [15]"]
idx = 2
for (mdat, label) in zip(rs_RPA, m_RPA, labels):
    print("\nPlotting ", label)
    handle = plot_mvsrs(rs_RPA, mdat, idx, label, '--', rs_HDL=rs_HDL, meff_HDL=meff_HDL)
    l1_handles.append(handle)
    idx = idx + 1
...
# m_VDMC.append(0.98)
print("\nPlotting our data")
plot_mvsrs(rs_VDMC, m_VDMC, idx, "", '-', rs_HDL=rs_HDL, meff_HDL=meff_HDL)
